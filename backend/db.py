...
class Track(Base):
    ...
    # СЛУЧАЙНЫЙ СОСТАВ КАДРА. Модель, предоставленная себе, тащит одного и
    # того же героя во все кадры подряд — клип выходит монотонным. С флагом
    # состав раскладывается по кадрам: где-то один герой, где-то двое, изредка
...
class Scene(Base):
    ...
    # Человек СНЯЛ галочку сам. Новое видео сцены встаёт в клип автоматически
    # (иначе оплаченный кадр молча не попадал в сборку), но отвергнутую сцену
    # авто-приём обязан оставить в покое.
    approved_manual = Column(Boolean, nullable=False, default=False)
    # Реальная длина готового ролика. Слот сцены и длина видео расходятся
    # (движок отдаёт свою), а обрезать надо по фактическим границам. Считаем
    # ffprobe один раз при появлении видео: гонять его на каждый ответ API
    # значит сотни запусков процесса на один экран.
    video_seconds = Column(Float, nullable=False, default=0)
    # Отпечаток цепочки кадров, из которой снято текущее видео: светофор
    # карточки сравнивает его с кадрами сейчас и честно говорит «устарело».
    video_src_sig = Column(String, nullable=False, default="")
