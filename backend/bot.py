...
готовый клип. Плюс ручной режим по сценам, «мои клипы», очки с тарифом и
партнёрский раздел.
...
1. Бот НЕ заводит свою авторизацию. Он опознаёт человека по tg_id, получает
   ...
   ОБЫЧНЫЕ публичные роуты. Поэтому аккаунт, очки, тариф, проекты и партнёрка
...
STR = {
    ...
    "ru": {
        ...
        "menu_points": "💎 Очки", "menu_settings": "⚙️ Настройки",
        ...
        "audio_too_big": (
            ...
            "оригинал в студии: аккаунт и очки те же:\n{url}"),
        ...
        "audio_got": ("🎧 <b>{title}</b> · {dur}\n"
                      ...
                      "Движок: <b>{engine}</b> · цена <b>{cost} очков</b> · "
                      "у тебя <b>{points}</b>.\n\nТеперь выбери стиль."),
        ...
        "confirm": ("🎬 <b>{title}</b>\n"
                    ...
                    "<b>{cost} очков</b> — останется <b>{left}</b>.\n"
        ...
        "no_points": "Не хватает очков: нужно <b>{need}</b>, есть <b>{have}</b>.",
        "buy_points": "⭐ Купить {n}+ очков", "upgrade": "💎 Сменить тариф",
        "clips_title": "🎬 <b>Мои клипы</b> — всего {n}",
        "clips_empty": "Клипов пока нет. Пришли трек — исправим.",
        "clip_row": "{i}. <b>{title}</b> · {dur} · {style}",
        "points_title": ("<b>{plan}</b>{until}\n<b>{points} очков</b> ≈ ещё "
                         "<b>{movies}</b> клипов\n\nЦена сцены на твоём тарифе: {engines}"),
        "buy": "⭐ Купить очки", "plans": "💎 Тарифы", "paysupport_btn": "🧾 Вопрос по оплате",
        "packs_title": "<b>Пакеты очков</b> — они не сгорают.",
        ...
        "paysupport": (
            ...
            "• Очки не сгорают и тратятся посценово.\n"
        ...
        "site_link": ("Ссылка в студию — аккаунт и очки те же:\n{url}"),
...
class Bot:
    ...
    def api_error_text(self, e: ApiError, lang: str) -> tuple[str, list[list[dict]]]:
        ...
        известна настоящая причина: кончились очки — показываем сколько нужно
    ...
    async def show_manual(self, tg_user: dict, chat_id: int, lang: str, track_id: int,
        ...
        ссылка в веб-студию: аккаунт и очки те же."""
        try:
            project = await self.api.req(tg_user, "GET", "/api/project")
        except Exception as e:  # noqa: BLE001
            await self.report_api_error(chat_id, lang, e)
    ...
    async def set_commands(self) -> None:
        cmds = {
            ...
            "ru": [("start", "начать заново"), ("new", "новый клип"), ("clips", "мои клипы"),
                   ("points", "очки и тариф"), ("partners", "партнёрская программа"),
