...
пользователю 120 токенов, а нам — ноль (наша подписка). Один платный мастеринг
у RoEx — это ~$2, то есть ~180 токенов живыми деньгами: дороже целого клипа за
...
def _roex_master(source_url: str, dst: str, style: str, loudness: str) -> str:
    ...
    что сервис вообще принял трек. Кредиты списывает уже финал, поэтому токены
...
def engines(reference: bool = False) -> list:
    ...
    price_hint — не цена в токенах (она в main.py), а маркер «нам это стоит
